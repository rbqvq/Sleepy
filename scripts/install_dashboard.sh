...
# Install program
{
    ...
    # Decompress package
    {
        ...
        [ -d "$TMP_DIR/custom" ] && cp -rf $TMP_DIR/custom /opt/$service/custom
        cp -rf $TMP_DIR/resources /opt/$service/resources
        [ -d "$TMP_DIR/examples/$PROGRAM" ] && cp -rf $TMP_DIR/examples/$PROGRAM /opt/$service/examples
...
ExecStart=/opt/$service/sleepy-dashboard --config config.json --log run.log
EOF
    }
}
